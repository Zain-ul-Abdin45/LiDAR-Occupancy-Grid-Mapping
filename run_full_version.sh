...
log "1/9  T1 — nuScenes, all 10 scenes, single-scan"
$PY main.py --dataset nuscenes --all-scenes --single-scan --eval --no-show --out output

log "2/9  T2 (PC-SBL beta=1) — nuScenes, all 10 scenes, single-scan"
$PY main.py --dataset nuscenes --all-scenes --single-scan --tier2 --beta 1.0 --eval --no-show --out output

log "3/9  Multi-frame sweep (T1+T2, w in {0,1,2,4}) — nuScenes, all 10 scenes"
$PY run_multiframe_benchmark.py

log "4/9  KITTI 3D Object — T1/T2 generalization benchmark (50 frames)"
$PY run_kitti_benchmark.py

log "5/9  KITTI Odometry — T1/T2 single+multi-frame benchmark (50 frames, seq 00)"
$PY run_kitti_odometry_benchmark.py --sequence 00 --n-frames 50 --window 2

log "6/9  Hyperparameter — acceleration ablation (rectangular submap grid)"
$PY run_accel_benchmark.py

log "7/9  Hyperparameter — angular sector ablation + precision metric"
$PY run_sector_benchmark.py

log "8/9  Report figures (tradeoff, coupling, convergence, beta sweep)"
$PY build_report_figures.py

log "9/9  Cross-dataset comparison graph (nuScenes vs KITTI 3D vs KITTI Odometry)"
